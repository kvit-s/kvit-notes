...
bool kvitNewtxCharterGeneratedPrototypeEnabled() {
  ...
  return value == nullptr || string(value) != "cm";
}

sptr<Atom> kvitNewtxVariantBlackboardAtom(const wstring& style, const wstring& arg) {
  if (!kvitNewtxCharterGeneratedPrototypeEnabled())
    return nullptr;

  const bool singleVariant = style == L"varmathbb" || style == L"vmathbb";
  const bool doubleVariant = style == L"vvmathbb";
  if (!singleVariant && !doubleVariant)
    return nullptr;

  const string prefix = doubleVariant ? "tx@Bbb" : "m@thbbch@r";
  if (arg == L"\\Gamma") return SymbolAtom::get(prefix + "Gamma");
  if (arg == L"\\Pi") return SymbolAtom::get(prefix + "Pi");
  if (arg == L"\\gamma") return SymbolAtom::get(prefix + "gamma");
  if (arg == L"\\pi") return SymbolAtom::get(prefix + "pi");
  return nullptr;
}
...
macro(textstyles) {
  ...
  if (auto atom = kvitNewtxVariantBlackboardAtom(style, args[1]); atom != nullptr)
    return atom;
