...
inline macro(vv) {
  // newtxmath declares \vv as \@ifstar{\vvstar}{\txvec}. The starred form
  // \vv*{x}{i} is \vvstar: {\txvec{#1}}_{\mkern-1mu#2}. With one declared
  // argument the starred call hands '*' in args[1] and leaves the two real
  // groups in the parser.
  if (args[1] == L"*") {
    auto base = sptrOf<NewtxVectorAtom>(tp.getArgument());
    auto sub = sptrOf<RowAtom>(
      sptrOf<SpaceAtom>(UnitType::mu, -1.f, 0.f, 0.f));
    sub->add(tp.getArgument());
    return sptrOf<ScriptsAtom>(base, sub, nullptr);
  }
  return sptrOf<NewtxVectorAtom>(Formula(tp, args[1], false)._root);
}
...
inline sptr<Atom> _newtx_group(
  ...
  return sptrOf<NewtxGroupAtom>(Formula(tp, args[1], false)._root, left, right, isOver);
}

inline sptr<Atom> _newtx_brace(TeXParser& tp, std::vector<std::wstring>& args, bool isOver) {
  return sptrOf<NewtxBraceAtom>(Formula(tp, args[1], false)._root, isOver);
}

inline macro(underbrace) {
  return _newtx_brace(tp, args, false);
}

inline macro(overbrace) {
  return _newtx_brace(tp, args, true);
}
...
inline macro(undergroup) {
  return _newtx_group(tp, args, "grouplu", "groupru", false);
}

inline macro(overgroup) {
  return _newtx_group(tp, args, "groupld", "grouprd", true);
}

inline macro(undergroupra) {
  return _newtx_group(tp, args, "grouplu", "grouprua", false);
}

inline macro(overgroupra) {
  return _newtx_group(tp, args, "groupld", "grouprda", true);
}

inline macro(undergroupla) {
  return _newtx_group(tp, args, "grouplua", "groupru", false);
}

inline macro(overgroupla) {
  return _newtx_group(tp, args, "grouplda", "grouprd", true);
}

inline macro(widering) {
  auto group = _newtx_group(tp, args, "groupld", "grouprd", true);
  return sptrOf<UnderOverAtom>(
    group,
    SymbolAtom::get("mathring"),
    UnitType::mu,
    2.5f,
    true,
    true
