...
#elif __has_include("../res/font/newtx-generated/kvit_newtx_zchmia.def.cpp") \
    && __has_include("../res/font/newtx-generated/kvit_newtx_ntxsy.def.cpp") \
    && __has_include("../res/font/newtx-generated/kvit_newtx_ntxsym.def.cpp") \
    && __has_include("../res/font/newtx-generated/kvit_newtx_ntxsyc.def.cpp") \
    && __has_include("../res/font/newtx-generated/kvit_newtx_ntxexx.def.cpp") \
    && __has_include("../res/font/newtx-generated/kvit_newtx_ntxexa.def.cpp")
#define KVIT_NEWTX_GENERATED_SYMBOLS_AVAILABLE 1
#else
...
const char* kvitNewtxDirectSymbolAlias(const wstring& command) {
  if (command == L"ne" || command == L"neq")
    return "neq";
  if (command == L"notin")
    return "notin";
  if (command == L"iint")
  ...
  if (command == L"iiiint")
    return "iiiint";
  if (command == L"colonequals")
    return "coloneqq";
  if (command == L"coloncolonequals")
    return "Coloneqq";
  if (command == L"equalscolon")
    return "eqqcolon";
  if (command == L"equalscoloncolon")
    return "Eqqcolon";
  if (command == L"colonsim")
    return "colonsim";
  if (command == L"coloncolonsim")
    return "Colonsim";
  if (command == L"colonapprox")
    return "colonapprox";
  if (command == L"coloncolonapprox")
    return "Colonapprox";
  if (command == L"Join")
    return "Join";
  return nullptr;
...
bool kvitNewtxDirectAccentCommand(const wstring& command) {
  return command == L"dddot" || command == L"ddddot";
}

const wchar_t* kvitNewtxComposedFormulaAlias(const wstring& command) {
  if (command == L"mappedfrom")
    return L"\\leftarrow\\mappedfromchar";
  if (command == L"longmappedfrom")
    return L"\\longleftarrow\\mappedfromchar";
  if (command == L"Mappedfrom")
    return L"\\Leftarrow\\Mappedfromchar";
  if (command == L"Longmappedfrom")
    return L"\\Longleftarrow\\Mappedfromchar";
  if (command == L"mmapsto")
    return L"\\mmapstochar\\rightarrow";
  if (command == L"longmmapsto")
    return L"\\mmapstochar\\longrightarrow";
  if (command == L"mmappedfrom")
    return L"\\leftarrow\\mmappedfromchar";
  if (command == L"longmmappedfrom")
    return L"\\longleftarrow\\mmappedfromchar";
  if (command == L"Mmapsto")
    return L"\\Mmapstochar\\Rightarrow";
  if (command == L"Longmmapsto")
    return L"\\Mmapstochar\\Longrightarrow";
...
sptr<Atom> kvitNewtxGeneratedDirectSymbol(const wstring& command) {
#if KVIT_NEWTX_GENERATED_SYMBOLS_AVAILABLE
  if (!kvitNewtxCharterGeneratedPrototypeEnabled())
    return nullptr;
  const char* symbol = kvitNewtxDirectSymbolAlias(command);
  if (symbol == nullptr)
    return nullptr;
  try {
    return SymbolAtom::get(symbol);
...
#if KVIT_NEWTX_GENERATED_SYMBOLS_AVAILABLE
  if (kvitNewtxCharterGeneratedPrototypeEnabled()
      && kvitNewtxDirectAccentCommand(command)) {
    return sptrOf<AccentedAtom>(getArgument(), wide2utf8(command));
  }
#endif

  if (sptr<Atom> generatedSymbol = kvitNewtxGeneratedDirectSymbol(command))
    return generatedSymbol;

#if KVIT_NEWTX_GENERATED_SYMBOLS_AVAILABLE
  if (kvitNewtxCharterGeneratedPrototypeEnabled()) {
    if (const wchar_t* formula = kvitNewtxComposedFormulaAlias(command))
      return Formula(*this, formula, false)._root;
  }
#endif
...
sptr<Atom> TeXParser::getScripts(wchar_t first) {
  ...
  auto* newtxBrace = dynamic_cast<NewtxBraceAtom*>(atom.get());
  if (newtxBrace != nullptr) {
    if (newtxBrace->isOver()) {
      if (sup != nullptr) {
        newtxBrace->addScript(sup);
        return sptrOf<ScriptsAtom>(atom, sub, nullptr);
      }
    } else if (sub != nullptr) {
      newtxBrace->addScript(sub);
      return sptrOf<ScriptsAtom>(atom, nullptr, sup);
    }
  }
...
void TeXParser::preprocess(wstring& cmd, Args& args, int& pos) {
  if (cmd == L"newcommand" || cmd == L"renewcommand") {
    ...
    KVIT_NEWTX_GENERATED_SYMBOLS_AVAILABLE
    && kvitNewtxCharterGeneratedPrototypeEnabled()
    && kvitNewtxDirectSymbolAlias(cmd) != nullptr
  ) {
    return;
