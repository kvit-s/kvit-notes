...
sptr<Box> kvitNewtxSymbolBox(Environment& env, const string& name) {
  return sptrOf<CharBox>(env.getTeXFont()->getChar(name, env.getStyle()));
}

sptr<Box> kvitNewtxKern(Environment& env, float mu) {
  return sptrOf<StrutBox>(SpaceAtom::getSize(UnitType::mu, mu, env), 0.f, 0.f, 0.f);
}

void kvitNewtxAddExtender(sptr<HBox>& box, Environment& env, const sptr<Box>& extender) {
  box->add(kvitNewtxKern(env, -0.5f));
  box->add(extender);
  box->add(kvitNewtxKern(env, -0.5f));
}

sptr<HBox> kvitNewtxExtenderFill(Environment& env, const sptr<Box>& extender, float fillWidth) {
  auto fill = sptrOf<HBox>();
  if (fillWidth <= PREC || extender->_width <= PREC) return fill;

  const float halfKern = SpaceAtom::getSize(UnitType::mu, -0.5f, env);
  const float extenderUnitWidth = max(extender->_width + 2.f * halfKern, extender->_width * 0.5f);
  while (fill->_width + extenderUnitWidth < fillWidth - extender->_width * 0.25f) {
    kvitNewtxAddExtender(fill, env, extender);
  }

  const float remaining = fillWidth - fill->_width;
  if (remaining > PREC) {
    fill->add(sptrOf<ScaleBox>(extender, remaining / extender->_width, 1.f));
  }
  return fill;
}
...
sptr<Box> NewtxGroupAtom::createGroupFill(Environment& env, float minWidth) const {
  sptr<Box> left = kvitNewtxSymbolBox(env, _left);
  sptr<Box> right = kvitNewtxSymbolBox(env, _right);
  sptr<Box> extender = kvitNewtxSymbolBox(env, "newtxbraceext");

  const float sideKern = SpaceAtom::getSize(UnitType::mu, -1.f, env);
  const float halfKern = SpaceAtom::getSize(UnitType::mu, -0.5f, env);
  const float extenderUnitWidth = max(extender->_width + 2.f * halfKern, extender->_width * 0.5f);
  const float minNaturalWidth = left->_width + right->_width + extender->_width + 2.f * sideKern;
  const float targetWidth = max(minWidth, minNaturalWidth);
  const float fillWidth = max(0.f, targetWidth - left->_width - right->_width - 2.f * sideKern);

  auto fill = sptrOf<HBox>();
  float width = 0.f;
  while (width + extenderUnitWidth < fillWidth - extender->_width * 0.25f) {
    kvitNewtxAddExtender(fill, env, extender);
    width = fill->_width;
  }

  const float remaining = fillWidth - fill->_width;
  if (remaining > PREC && extender->_width > PREC) {
    auto scaled = sptrOf<ScaleBox>(extender, remaining / extender->_width, 1.f);
    fill->add(scaled);
  }

  auto group = sptrOf<HBox>();
  group->add(left);
  group->add(sptrOf<StrutBox>(sideKern, 0.f, 0.f, 0.f));
  group->add(fill);
  group->add(sptrOf<StrutBox>(sideKern, 0.f, 0.f, 0.f));
  group->add(right);
  return group;
...
sptr<Box> NewtxGroupAtom::createFallbackBox(Environment& env) const {
  const string symbol = _over ? "lgroup" : "rgroup";
  OverUnderDelimiter fallback(
    _base,
    nullptr,
  ...
  return fallback.createBox(env);
}

sptr<Box> NewtxGroupAtom::createBox(Environment& env) {
  sptr<Box> base = (_base == nullptr ? sptrOf<StrutBox>(0.f, 0.f, 0.f, 0.f) : _base->createBox(env));
  sptr<Box> group;
  try {
    group = createGroupFill(env, base->_width);
  } catch (ex_symbol_mapping_not_found&) {
    return createFallbackBox(env);
  }

  const float width = max(base->_width, group->_width);
  if (width - base->_width > PREC) base = sptrOf<HBox>(base, width, Alignment::center);
  if (width - group->_width > PREC) group = sptrOf<HBox>(group, width, Alignment::center);

  auto vbox = sptrOf<VBox>();
  if (_over) {
    vbox->add(SpaceAtom(UnitType::point, 0.f, 0.4f, 0.f).createBox(env));
    vbox->add(group);
    vbox->add(SpaceAtom(UnitType::point, 0.f, 1.5f, 0.f).createBox(env));
    vbox->add(base);
    const float total = vbox->_height + vbox->_depth;
    vbox->_height = total - base->_depth;
    vbox->_depth = base->_depth;
    ...
    vbox->add(SpaceAtom(UnitType::point, 0.f, 1.5f, 0.f).createBox(env));
    vbox->add(group);
    vbox->add(SpaceAtom(UnitType::point, 0.f, 0.5f, 0.f).createBox(env));
    const float total = vbox->_height + vbox->_depth;
    vbox->_height = base->_height;
    vbox->_depth = total - base->_height;
  }
...
sptr<Box> NewtxBraceAtom::createBraceFill(Environment& env, float minWidth) const {
  sptr<Box> start = kvitNewtxSymbolBox(env, _over ? "braceld" : "bracelu");
  sptr<Box> centerLeft = kvitNewtxSymbolBox(env, _over ? "braceru" : "bracerd");
  sptr<Box> centerRight = kvitNewtxSymbolBox(env, _over ? "bracelu" : "braceld");
  sptr<Box> end = kvitNewtxSymbolBox(env, _over ? "bracerd" : "braceru");
  sptr<Box> extender = kvitNewtxSymbolBox(env, "newtxbraceext");
  ...
  const float fixedWidth =
    start->_width + centerLeft->_width + centerRight->_width + end->_width + 4.f * sideKern;
  const float minNaturalWidth = fixedWidth + 2.f * extenderUnitWidth;
  const float targetWidth = max(minWidth, minNaturalWidth);
  const float fillWidth = max(0.f, targetWidth - fixedWidth) / 2.f;

  auto brace = sptrOf<HBox>();
  brace->add(start);
  brace->add(kvitNewtxKern(env, -1.f));
  brace->add(kvitNewtxExtenderFill(env, extender, fillWidth));
  brace->add(kvitNewtxKern(env, -1.f));
  brace->add(centerLeft);
  brace->add(centerRight);
  brace->add(kvitNewtxKern(env, -1.f));
  brace->add(kvitNewtxExtenderFill(env, extender, fillWidth));
  brace->add(kvitNewtxKern(env, -1.f));
  brace->add(end);
...
sptr<Box> NewtxBraceAtom::createFallbackBox(Environment& env) const {
  OverUnderDelimiter fallback(
    _base,
    _script,
    SymbolAtom::get(_over ? "lbrace" : "rbrace"),
    UnitType::ex,
    0.f,
    _over
  );
...
sptr<Box> NewtxBraceAtom::createBox(Environment& env) {
  sptr<Box> base = (_base == nullptr ? sptrOf<StrutBox>(0.f, 0.f, 0.f, 0.f) : _base->createBox(env));
  sptr<Box> brace;
  try {
    brace = createBraceFill(env, base->_width);
  } catch (ex_symbol_mapping_not_found&) {
    return createFallbackBox(env);
  }

  sptr<Box> script(nullptr);
  if (_script != nullptr) {
    script = _script->createBox((_over ? *(env.supStyle()) : *(env.subStyle())));
  }

  float width = max(base->_width, brace->_width);
  if (script != nullptr) width = max(width, script->_width);
  if (width - base->_width > PREC) base = sptrOf<HBox>(base, width, Alignment::center);
  if (width - brace->_width > PREC) brace = sptrOf<HBox>(brace, width, Alignment::center);
  if (script != nullptr && width - script->_width > PREC) {
    script = sptrOf<HBox>(script, width, Alignment::center);
  }

  auto vbox = sptrOf<VBox>();
  if (_over) {
    if (script != nullptr) vbox->add(script);
    vbox->add(brace);
    vbox->add(base);
    const float total = vbox->_height + vbox->_depth;
    vbox->_height = total - base->_depth;
...
sptr<Box> NewtxVectorAtom::createVectorFill(Environment& env, float minWidth) const {
  sptr<Box> extender = kvitNewtxSymbolBox(env, "rVec@x");
  sptr<Box> head = kvitNewtxSymbolBox(env, "rVec@h");

  const float leftKern = SpaceAtom::getSize(UnitType::mu, 2.f, env);
  const float joinKern = SpaceAtom::getSize(UnitType::mu, -1.5f, env);
  const float leaderKern = SpaceAtom::getSize(UnitType::mu, -0.4f, env);
  const float extenderUnitWidth = max(extender->_width + 2.f * leaderKern, extender->_width * 0.5f);
  const float minNaturalWidth = leftKern + extender->_width + head->_width + 2.f * joinKern;
  const float targetWidth = max(minWidth, minNaturalWidth);
  const float fillWidth = max(0.f, targetWidth - leftKern - extender->_width - head->_width - 2.f * joinKern);

  auto fill = sptrOf<HBox>();
  float width = 0.f;
  while (width + extenderUnitWidth < fillWidth - extender->_width * 0.25f) {
    fill->add(sptrOf<StrutBox>(leaderKern, 0.f, 0.f, 0.f));
    fill->add(extender);
    fill->add(sptrOf<StrutBox>(leaderKern, 0.f, 0.f, 0.f));
    width = fill->_width;
  }
  ...
  auto vector = sptrOf<HBox>();
  vector->add(sptrOf<StrutBox>(leftKern, 0.f, 0.f, 0.f));
  vector->add(extender);
  vector->add(sptrOf<StrutBox>(joinKern, 0.f, 0.f, 0.f));
  vector->add(fill);
  vector->add(sptrOf<StrutBox>(joinKern, 0.f, 0.f, 0.f));
  vector->add(head);
  return vector;
...
sptr<Box> NewtxVectorAtom::createFallbackBox(Environment& env) const {
  sptr<Box> base = (_base == nullptr ? sptrOf<StrutBox>(0.f, 0.f, 0.f, 0.f) : _base->createBox(env));
  sptr<Box> arrow = XLeftRightArrowFactory::create(false, env, base->_width);

  auto vbox = sptrOf<VBox>();
  vbox->add(arrow);
  vbox->add(sptrOf<HBox>(base, arrow->_width, Alignment::center));
  const float total = vbox->_height + vbox->_depth;
  vbox->_depth = base->_depth;
  vbox->_height = total - base->_depth;
  return vbox;
...
sptr<Box> NewtxVectorAtom::createBox(Environment& env) {
  sptr<Box> base = (_base == nullptr ? sptrOf<StrutBox>(0.f, 0.f, 0.f, 0.f) : _base->createBox(env));
  sptr<Box> vector;
  try {
    vector = createVectorFill(env, base->_width);
  } catch (ex_symbol_mapping_not_found&) {
    return createFallbackBox(env);
  }

  const float width = max(base->_width, vector->_width);
  if (width - base->_width > PREC) base = sptrOf<HBox>(base, width, Alignment::center);
  if (width - vector->_width > PREC) vector = sptrOf<HBox>(vector, width, Alignment::center);

  auto vbox = sptrOf<VBox>();
  vbox->add(SpaceAtom(UnitType::ex, 0.f, 0.2f, 0.f).createBox(env));
  vbox->add(vector);
  vbox->add(SpaceAtom(UnitType::mu, 0.f, 1.5f, 0.f).createBox(env));
  vbox->add(base);
  const float total = vbox->_height + vbox->_depth;
  vbox->_height = total - base->_depth;
  vbox->_depth = base->_depth;
