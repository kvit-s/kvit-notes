...
class NewtxGroupAtom : public Atom {
private:
  sptr<Atom> _base;
  std::string _left;
  std::string _right;
  ...
  sptr<Box> createGroupFill(Environment& env, float minWidth) const;
  sptr<Box> createFallbackBox(Environment& env) const;

public:
  NewtxGroupAtom() = delete;
  ...
  NewtxGroupAtom(
    ...
    std::string right,
    bool over
  ) : _base(base), _left(std::move(left)), _right(std::move(right)), _over(over) {
    _type = AtomType::inner;
  }

  sptr<Box> createBox(Environment& env) override;

  __decl_clone(NewtxGroupAtom)
};

class NewtxBraceAtom : public Atom {
private:
  sptr<Atom> _base;
  sptr<Atom> _script;
  bool _over;

  sptr<Box> createBraceFill(Environment& env, float minWidth) const;
  sptr<Box> createFallbackBox(Environment& env) const;

public:
  NewtxBraceAtom() = delete;

  NewtxBraceAtom(const sptr<Atom>& base, bool over)
    : _base(base), _script(nullptr), _over(over) {
    _type = AtomType::inner;
  }
  ...
  __decl_clone(NewtxBraceAtom)
};

class NewtxVectorAtom : public Atom {
private:
  sptr<Atom> _base;

  sptr<Box> createVectorFill(Environment& env, float minWidth) const;
  sptr<Box> createFallbackBox(Environment& env) const;

public:
  NewtxVectorAtom() = delete;

  explicit NewtxVectorAtom(const sptr<Atom>& base) : _base(base) {
    _type = AtomType::ordinary;
  }

  sptr<Box> createBox(Environment& env) override;

  __decl_clone(NewtxVectorAtom)
};
