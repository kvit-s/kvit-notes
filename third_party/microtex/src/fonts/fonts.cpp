...
// newtxmath's lmsntxsy.fd gives the symbols family genuine optical script
// masters: ntxsy5 below 6.3pt and ntxsy7 from 6.3pt up to 8.6pt. MicroTeX has
// no per-size font sets; it scales one master by the script factors 0.7/0.5.
// Because the ntxsy5/ntxsy7 TFMs are 10pt-design fonts meant to be loaded
// scaled down, swapping the font id at script styles while keeping those
// factors reproduces LaTeX's selection exactly for 10pt-class base sizes.
int kvitNewtxOpticalScriptFontId(int fontId, tex::TexStyle style) {
  if (style < tex::TexStyle::script) return fontId;
  if (!kvitNewtxCharterGeneratedPrototypeEnabled()) return fontId;
  static const int sy = tex::FontInfo::__id("kvit_newtx_ntxsy");
  static const int sy7 = tex::FontInfo::__id("kvit_newtx_ntxsy7");
  static const int sy5 = tex::FontInfo::__id("kvit_newtx_ntxsy5");
  static const int bsy = tex::FontInfo::__id("kvit_newtx_ntxbsy");
  static const int bsy7 = tex::FontInfo::__id("kvit_newtx_ntxbsy7");
  static const int bsy5 = tex::FontInfo::__id("kvit_newtx_ntxbsy5");
  const bool scriptScript = style >= tex::TexStyle::scriptScript;
  if (fontId == sy) return scriptScript ? sy5 : sy7;
  if (fontId == bsy) return scriptScript ? bsy5 : bsy7;
  return fontId;
}
...
Char DefaultTeXFont::getChar(const CharFont& c, TexStyle style) {
  CharFont cf = c;
  cf.fontId = kvitNewtxOpticalScriptFontId(cf.fontId, style);
  cf.boldFontId = kvitNewtxOpticalScriptFontId(cf.boldFontId, style);
