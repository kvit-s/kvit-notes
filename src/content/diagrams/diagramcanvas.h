...
class DiagramCanvas : public QQuickPaintedItem
{
    ...
    // current source; a last-good preview of invalid source gates off.
    ...
    // Manual arrangement: available for rendered flowcharts; a pos line
...
public:
    ...
    // Save the current scene as a PNG at the given device scale (the "Save
    // PNG" action). Renders with the currently bound theme colors on the
    ...
    // ---- Hit-testing and selection (item coordinates) ----
    ...
    // ---- Gesture wrappers -----------------------------------------
    // Each computes the gesture through the edit engine and returns the
    // full new fence source (one undo step via updateContent), or an empty
    // string with gestureError set when the gesture is refused.
    ...
    // Move the selected message down/up (+1/-1) one message, or
    // the selected participant right/left one declaration.
    ...
    // ---- Anchor drag: ghost edge, committed on drop ----
    ...
    // ---- Node dragging (item coordinates; commit on release) ----
    ...
    // Commit: the full-snapshot pos-line write. Returns the new fence
...
private:
    ...
    // Drag state: scene coordinates; ghost committed on release.
